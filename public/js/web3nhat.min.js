if (typeof jQuery === "undefined") {
    ...
    d(document).on("click", ".tab .nav-tabs li a", function(f) {
        f.preventDefault();
        c.call(d(this), "show")
    })
}(jQuery); + function(h) {
...
(function(a) {
    a.fn.LoPopUp = function(d) {
        ...
        if (!a("#IDLOpopup").html()) {
            a("body").append('<div id="IDLOpopup" rel="0" class="LOpopup"><div class="LOpopupDialogClass"><div class="LOpopupDialogHead">TiĂªu Ä‘á»</div> <span  class="closepopup" onclick="closeAllPopup($(this));; return false;">X</span><div class="loading"></div> <div class="LOpopupDialogBody"></div></div></div>')
        }
        if (e.clearBefore) {
            a(this).find(".LOpopupDialogBody").html("")
        }
        if (!e.closePopup) {
            a(this).show()
        } else {
        ...
        if (e.loadingStart) {
            a(this).find(".loading").show()
        }
        if (a.trim(e.contentHtml) != "") {
            if (a.trim(e.timeoutAppend) == "") {
                a(this).find(".LOpopupDialogBody").html(e.contentHtml);
                a(this).find(".loading").hide();
                a(this).find(".LOpopupDialogBody").slideDown("slow");
                a(this).find(".loading").hide();
                if (a.isFunction(e.afterSuccess)) {
                    e.afterSuccess.call(this)
                }
            } else {
                var c = a(this);
                setTimeout(function() {
                    c.find(".LOpopupDialogBody").html(e.contentHtml);
                    c.find(".loading").hide();
                    c.find(".LOpopupDialogBody").slideDown("slow");
                    c.find(".loading").hide();
                    if (a.isFunction(e.afterSuccess)) {
                        e.afterSuccess.call(this)
                    }
        ...
        if (a.trim(e.title) != "") {
            a(this).find(".LOpopupDialogHead").html(e.title)
        }
        if (a.trim(e.top) != "") {
            a(this).css("top", e.top)
        }
        if (a.trim(e.maxwidth) != "") {
            a(this).find(".LOpopupDialogClass ").css("max-width", e.maxwidth)
        }
        if (a.trim(e.minwidth) != "") {
            a(this).find(".LOpopupDialogClass ").css("min-width", e.minwidth)
        }
        if (a.trim(e.maxheight) != "") {
            a(this).find(".LOpopupDialogClass ").css("max-height", e.maxheight)
        }
        if (a.trim(e.minheight) != "") {
            a(this).find(".LOpopupDialogClass ").css("max-height", e.minheight)
        }
        var b = this;
        setTimeout(function() {
            var f = a(b).find(".LOpopupDialogHead");
            if (a.trim(e.maxheight) != "") {
                a(b).find(".LOpopupDialogBody").css("max-height", (parseInt(e.maxheight) - 8 - f.outerHeight(true)) + "px")
            } else {
                a(b).find(".LOpopupDialogBody").css("max-height", (a(b).find(".LOpopupDialogClass").height() - f.outerHeight(true)) + "px")
            }
        }, 10);
        return this
    }
...
function createPopup(a) {
    ...
    $("#IDLOpopup").clone().attr("id", a.popupId).after("#IDLOpopup");
    $("#" + settings.popupId).LoPopUp(a)
}

function parseJsonErrors(a, b) {
    var c = $.parseJSON(a);
    if (b) {
        b.find(".errorMessage").hide();
        b.find(".poError").removeClass("poError");
        $.each(c, function(d, e) {
            b.find("#" + d + "_em_").text(e);
            b.find("#" + d + "_em_").show();
            b.find("#" + d + "_em_").parent().addClass("poError")
        })
    } else {
        jQuery(".errorMessage").hide();
        jQuery(".poError").removeClass("poError");
        $.each(c, function(d, e) {
            jQuery("#" + d + "_em_").text(e);
            jQuery("#" + d + "_em_").show();
            jQuery("#" + d + "_em_").parent().addClass("poError")
        })
    }
}

function w3ShowLoading(c, f, a, i) {
    if (!f) {
        f = "right"
    }
    if (!a) {
    ...
    if (e) {
        ...
        if (!$("#objloading").html()) {
            $("body").append('<div id="objloading" style="position: absolute; top: 0px; left: 0px;z-index: 9999"><div id="imgloading" style="position: absolute; top:' + d + "px; left:" + g + 'px;"><img style="display: block;" src="img/wating.gif" /></div></div>')
        }
        $("#imgloading").css({
            top: d + "px",
            left: g + "px"
...
function updateCountCart(b, c) {
    if (!b) {
        return false
    }
    if (isNumber(b)) {
        jQuery(".countCart").html(b);
    ...
    if (isNumber(c)) {
        ...
        jQuery(a).html(c);
        jQuery(a).show();
        jQuery(a).removeClass("shake");
        setTimeout(function() {
            jQuery(a).addClass("shake")
        }, 200)
    }
}
jQuery(document).ready(function() {
    var c = jQuery(".container-out");
    if (c.length) {
        var d = parseInt(jQuery(".container:first").outerWidth());
        if (d) {
    ...
    if (jQuery(".product-detail .product-detail-img").length) {
        if (jQuery(".product-detail .product-detail-img").hasClass("mobile")) {
            ...
            console.log();
            $(".product-detail .product-detail-img a.product-img-large").colorbox({
                rel: "product-img-large",
                innerHeight: e - 20,
                innerWidth: "100%"
            });
            $(".product-detail .product-detail-img a.product-img-small").colorbox({
                rel: "product-img-small",
                innerHeight: e - 20,
                innerWidth: "100%"
            })
        } else {
        ...
        jQuery(".product-detail .product-detail-img a.product-img-small").on("mouseover", function() {
            ...
            var f = jQuery(this).find("img").attr("data-thumb-image");
            if (g) {
                var h = jQuery(this).closest(".product-detail-img");
                h.find(".product-img-main a.product-img-large").attr("href", g);
                if (!f) {
                    h.find(".product-img-main a.product-img-large img").attr("src", i.replace("/s50_50/", "/s" + thumb_img_w + "_" + thumb_img_h + "/"))
                } else {
                    h.find(".product-img-main a.product-img-large img").attr("src", f)
                }
            }
            return false
        })
    ...
    jQuery(".addtocart").on("click", function() {
        ...
        var h = g.attr("data-params");
        var l = "";
        if (h) {
            var k = jQuery(h);
            if (k.find(".product-attr").length) {
                var f = true;
                var m = "";
                k.find(".product-attr").each(function() {
                    if (!$(this).find(".attrConfig-input").val()) {
                        f = false;
                        if (!m) {
                            m = $(this).attr("attr-title")
                        } else {
                            m += ", " + $(this).attr("attr-title")
                        }
                    }
                ...
                if (!f) {
                    var j = k.find(".product-attr-error");
                    j.show();
                    j.find("b").html(m);
                    return false
                } else {
                    k.find(".product-attr-error").hide()
                }
            }
            if (k.length) {
                l = k.find("input,select,textarea").serialize()
            }
        }
        jQuery.ajax({
            type: "post",
            ...
            dataType: "JSON",
            beforeSend: function() {
                w3ShowLoading(g, "right", 15, 0)
            },
            success: function(n) {
                if (n.code == "200") {
                    updateCountCart(n.total, n.products);
                    if (g.hasClass("refreshcart")) {
                        $(".cart").append(n.cart)
                    } else {
                        if (!g.hasClass("noredirect") && n.redirect) {
                            window.location.href = n.redirect
                        } else {
                            if (n.cart && n.cartTitle) {
                                $(document).LoPopUp({
                                    title: n.cartTitle,
                                    clearBefore: true,
                                    clearAfter: true,
    ...
    jQuery(document).on("mouseover", ".get-product-detail", function() {
        ...
        var i = g["target-detail"];
        if (!i || !f.find(i).length) {
            return false
        }
        if ($.trim(f.find(i).html()) != "") {
            return false
        }
        if (f.hasClass("block")) {
            return false
        ...
        jQuery.ajax({
            type: "post",
            data: {
                attributes: JSON.stringify(g)
            },
            url: h,
            dataType: "JSON",
            beforeSend: function() {
                if (g["show-loading"] != "false") {
                    w3ShowLoading(f, "right", 0, 0)
                }
            },
            success: function(j) {
                if (j.code == "200") {
                    if (j.html) {
                        f.find(i).html(j.html)
                    }
                }
                w3HideLoading();
                f.removeClass("block")
...
(function(a) {
    a.fn.getAttributes = function() {
        var b = {};
        if (this.length) {
            a.each(this[0].attributes, function(d, c) {
                b[c.name] = c.value
            })
        }
        return b
...
(function(d) {
    var c;
    d.fn.printThis = function(j) {
        c = d.extend({}, d.fn.printThis.defaults, j);
        var i = this instanceof jQuery ? this : d(this);
        var l = "printThis-" + (new Date()).getTime();
        if (window.location.hostname !== document.domain && navigator.userAgent.match(/msie/i)) {
            ...
            var k = d("<iframe id='" + l + "' name='printIframe' />");
            k.appendTo("body")
        }
        var m = d("#" + l);
        if (!c.debug) {
        ...
        setTimeout(function() {
            var u = m.contents(),
                p = u.find("head"),
                r = u.find("body");
            p.append('<base href="' + document.location.protocol + "//" + document.location.host + '">');
            if (c.importCSS) {
                d("link[rel=stylesheet]").each(function() {
                    var v = d(this).attr("href");
                    if (v) {
                        var w = d(this).attr("media") || "all";
                        p.append("<link type='text/css' rel='stylesheet' href='" + v + "' media='" + w + "'>")
                    }
                })
            }
            if (c.importStyle) {
                d("style").each(function() {
                    d(this).clone().appendTo(p)
                })
            }
            if (c.pageTitle) {
                p.append("<title>" + c.pageTitle + "</title>")
            }
            if (c.loadCSS) {
                if (d.isArray(c.loadCSS)) {
                    jQuery.each(c.loadCSS, function(v, w) {
                        p.append("<link type='text/css' rel='stylesheet' href='" + this + "'>")
                    })
                } else {
                    p.append("<link type='text/css' rel='stylesheet' href='" + c.loadCSS + "'>")
                }
            }
            if (c.header) {
                r.append(c.header)
            ...
            if (c.formValues) {
                ...
                if (t.length) {
                    t.each(function() {
                        var y = d(this),
                            z = d(this).attr("name"),
                            v = y.is(":checkbox") || y.is(":radio"),
                            w = u.find('input[name="' + z + '"]'),
                            x = y.val();
                        if (!v) {
                            w.val(x)
                        } else {
                            if (y.is(":checked")) {
                                if (y.is(":checkbox")) {
                                    ...
                                    if (y.is(":radio")) {
                                        u.find('input[name="' + z + '"][value=' + x + "]").attr("checked", "checked")
                                    }
                                }
                            }
                        }
                ...
                if (q.length) {
                    q.each(function() {
                        var w = d(this),
                            x = d(this).attr("name"),
                            v = w.val();
                        u.find('select[name="' + x + '"]').val(v)
                    })
                }
                var s = i.find("textarea");
                if (s.length) {
                    s.each(function() {
                        var w = d(this),
                            x = d(this).attr("name"),
                            v = w.val();
                        u.find('textarea[name="' + x + '"]').val(v)
                    })
                }
            }
            if (c.removeInline) {
                if (d.isFunction(d.removeAttr)) {
                    u.find("body *").removeAttr("style")
                } else {
                    u.find("body *").attr("style", "")
                }
            }
            setTimeout(function() {
                if (m.hasClass("MSIE")) {
                    window.frames.printIframe.focus();
                    p.append("<script>  window.print(); <\/script>")
                } else {
                    m[0].contentWindow.focus();
                    m[0].contentWindow.print()
                }
                if (!c.debug) {
                    setTimeout(function() {
                        m.remove()
    ...
    jQuery.fn.outer = function() {
        return d(d("<div></div>").html(this.clone())).html()
    };
    if (typeof g === "undefined" || !g) {
        var g = "../public/img/noimage.jpeg"
    }
    jQuery("img").error(function() {
        jQuery(this).attr("data-src", jQuery(this).attr("src"));
        jQuery(this).attr("src", g)
    });
    ...
    function e() {
        var i = false;
        jQuery("." + h).each(function(j, l) {
            var k = jQuery(l).attr(a);
            if (k) {
                jQuery(l).attr(f, k)
            }
            jQuery(l).removeClass(h);
            i = true;
            return false
        });
        return i
    }
    if (jQuery("img." + h).length) {
        jQuery("img." + h).lazyload()
    }
})(jQuery);
var w3nPublic = {};
...
w3nPublic.addParamToUrl = function(b, c, d) {
    var a = (b.indexOf("?") > -1) ? "&" : "?";
    return b + a + c + "=" + d
};